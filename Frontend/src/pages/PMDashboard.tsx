...
const PMDashboard = () => {
  ...
  const [loading, setLoading] = useState(false)
    ...
    const fetchUsers = async () => {
        ...
        setDevelopers(res.data.filter((u: User) => u.role === 'DEVELOPER'))
    }
    ...
    if (!projectName || !projectClientId || loading) return
    setLoading(true)
    try {
      ...
      setProjectName(''); setProjectDesc(''); setProjectClientId('')
  ...
  const createTask = async () => {
    ...
    const overdueTasks = selectedTasks.filter(t => t.status === 'OVERDUE').length
    ...
    const formStyle: React.CSSProperties = {
        background: 'white', padding: 32, borderRadius: 8, width: 400
    }
        ...
        <div style={{ minHeight: '100vh', background: '#f9fafb' }}>
            ...
            <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
                ...
                <div style={{ display: 'flex', gap: 16, marginBottom: 24 }}>
                    <div style={{ background: 'white', padding: 16, borderRadius: 8, border: '1px solid #e5e7eb', flex: 1 }}>
                        ...
                        <p style={{ fontSize: 32, margin: 0, color: '#4f46e5' }}>{totalTasks}</p>
                    </div>
                    <div style={{ background: 'white', padding: 16, borderRadius: 8, border: '1px solid #e5e7eb', flex: 1 }}>
                        <h4 style={{ margin: 0 }}>Overdue</h4>
                        <p style={{ fontSize: 32, margin: 0, color: '#ef4444' }}>{overdueTasks}</p>
                    </div>
                </div>
            ...
            {selectedTasks.length === 0 && (
              <div style={{ textAlign: 'center', padding: 48, background: 'white', borderRadius: 8, border: '1px solid #e5e7eb' }}>
                <p style={{ color: '#6b7280', marginBottom: 16 }}>No tasks here yet.</p>
